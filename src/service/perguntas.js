export const verificarInputs = (pergunta, alternativas, alternativasTemp, resposta, categoria, dificuldade, listaPerguntas) => {
    if (!pergunta) {
        throw new Error('Digite uma pergunta válida');
    }

    if (!alternativas[0]) {
        throw new Error('Digite a primeira alternativa');
    }

    if (!alternativas[1]) {
        throw new Error('Digite a segunda alternativa');
    }

    if (!alternativas[2]) {
        throw new Error('Digite a terceira alternativa');
    }

    if (!alternativas[3]) {
        throw new Error('Digite a quarta alternativa');
    }
    ...
    if (!dificuldade) {
        throw new Error('Escolha uma dificuldade');
    }

    if (listaPerguntas.filter(l => l.pergunta == pergunta).length > 0) {
        throw new Error('Pergunta ja existe');
    }

    if (alternativasTemp.filter(a => a === alternativas[0]).length > 1) {
        throw new Error('Alternativa ja existe');
    }
    ...
    if (!alternativasTemp.find(a => a === resposta)) {
        throw new Error('Resposta não confere com as alternativas');
    }
}

export const inicializarPerguntasDefault = (listaPerguntas) => {
    // const lista = [
        // {
